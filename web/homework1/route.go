...
func (r *router) findRoute(method string, path string) (*matchInfo, bool) {
	...
	pathMatchInfo := &matchInfo{}
	for _, s := range segments {
		var child *node
		child, ok = root.childOf(s)
		if !ok {
			if root.nType == nodeTypeAny {
				pathMatchInfo.n = root
				return pathMatchInfo, true
			}
			return nil, false
		}
		if child.paramName != "" {
			pathMatchInfo.addValue(child.paramName, s)
		}
		root = child
	}
	pathMatchInfo.n = root
...
func (n *node) childOrCreate(path string) *node {
	if path == "*" {
		...
		if n.starChild == nil {
			n.starChild = &node{path: path, nType: nodeTypeAny}
		}
		return n.starChild
	}
	...
	if !ok {
		child = &node{path: path, nType: nodeTypeStatic}
...
func (n *node) childOrCreateReg(path string, expr string, paramName string) *node {
	...
	if n.regChild != nil {
		...
		n.regChild = &node{path: path, paramName: paramName, regExpr: regExpr, nType: nodeTypeReg}
	}
	return n.regChild
}
