...
func Test_router_AddRoute(t *testing.T) {
			...
			http.MethodGet: {
				...
				children: map[string]*node{
					"user": {
						path: "user",
						children: map[string]*node{
							"home": {path: "home", handler: mockHandler, nType: nodeTypeStatic},
						},
						handler: mockHandler,
					...
					"order": {
						path: "order",
						children: map[string]*node{
							"detail": {path: "detail", handler: mockHandler, nType: nodeTypeStatic},
						},
						starChild: &node{path: "*", handler: mockHandler, nType: nodeTypeAny},
						nType:     nodeTypeStatic,
					},
					"param": {
						path: "param",
						paramChild: &node{
							...
							children: map[string]*node{"detail": {path: "detail", handler: mockHandler, nType: nodeTypeStatic}},
							handler:  mockHandler,
							nType:    nodeTypeParam,
						},
					},
			...
			http.MethodPost: {
				...
				children: map[string]*node{
					"order": {path: "order", children: map[string]*node{
						"create": {path: "create", handler: mockHandler, nType: nodeTypeStatic},
					}},
					"login": {path: "login", handler: mockHandler, nType: nodeTypeStatic},
				},
				nType: nodeTypeStatic,
			},
			http.MethodDelete: {
	...
	// 根节点重复注册
	r.addRoute(http.MethodGet, "/", mockHandler)
	assert.PanicsWithValue(t, "web: 路由冲突path: [/]", func() {
		r.addRoute(http.MethodGet, "/", mockHandler)
	})
	// 普通节点重复注册
	r.addRoute(http.MethodGet, "/a/b/c", mockHandler)
...
func (n *node) equal(y *node) (string, bool) {
	...
	if n.nType != y.nType {
		return fmt.Sprintf("%s 节点类型不相等 x %d, y %d", n.path, n.nType, y.nType), false
	}
